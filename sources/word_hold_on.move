...
module overmind::word_hold_on {
    ...
    */
    fun init_module(overmind: &signer) {
        ...
        let (resource_signer, resource_cap) = account::create_resource_account(overmind, SEED);
        ...
        coin::transfer<AptosCoin>(overmind, signer::address_of(&resource_signer), PRIZE);
    ...
    */
    public entry fun guess_word(player: &signer, word: String) acquires State, Player {
        ...
        if(hash::sha3_256(bcs::to_bytes(&word)) == ANSWER_HASH){
    ...
    */
    inline fun submit_correct_answer(player_resource: &mut Player, player_address: address) {
        ...
        coin::transfer<AptosCoin>(&res_signer, player_address, PRIZE);
    ...
    */
    inline fun add_guess_to_attempts(player_resource: &mut Player, player_address: &address, word: String) {
        ...
        while(i < string::length(&word)){
            let letter = string::sub_string(&word, i, i+1);
            let result = (get_letter_hash(i) == hash::sha3_256(*string::bytes(&letter)));
            string::append(&mut letter,string_utils::to_string_with_integer_types(&i));
            string::append(&mut letter,string_utils::to_string_with_canonical_addresses(player_address));
            string::append(&mut letter,string_utils::to_string_with_integer_types(&time_now));
            let composition = hash::sha3_256(bcs::to_bytes(&letter));
            simple_map::add(&mut map, composition, result);
            i = i+1;
        };
        ...
        simple_map::add(&mut player_resource.attempts, attempt, map);
    ...
    */
    inline fun get_letter_hash(index: u64): vector<u8> {
        ...
        assert!(index < 8, ELetterIndexOutOfBounds);
        let answers = vector[FIRST_LETTER_HASH, SECOND_LETTER_HASH, THIRD_LETTER_HASH, FOURTH_LETTER_HASH, FIFTH_LETTER_HASH, SIXTH_LETTER_HASH, SEVENTH_LETTER_HASH, EIGHTH_LETTER_HASH];
        let output = *vector::borrow(&answers, index);
        output
    }
